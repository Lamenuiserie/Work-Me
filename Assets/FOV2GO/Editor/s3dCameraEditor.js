...
class s3dCameraEditor extends Editor {
    ...
    function OnInspectorGUI () {
		...
		if (foldout2) {
	    	...
	    	if (target.format3D == 0 || target.format3D == 2) {	// side by side
	    		if (target.format3D == 0) {
	    		...
	    		} else if (target.format3D == 2) {
	    			target.overUnderStretched = EditorGUILayout.Toggle(GUIContent("Stretched","For 3D TV frame-compatible format"), target.overUnderStretched);
	    		}
	    		...
	    		target.anaglyphOptions = EditorGUILayout.EnumPopup(GUIContent("Anaglyph Mode","Anaglyph color formats"), target.anaglyphOptions);
				...
				target.sceneScreenL = EditorGUILayout.ObjectField(GUIContent("Scene Screen L"," "),target.sceneScreenL,RenderTexture,allowSceneObjects);
				target.sceneScreenR = EditorGUILayout.ObjectField(GUIContent("Scene Screen R"," "),target.sceneScreenR,RenderTexture,allowSceneObjects);
	    	}
			if (target.useStereoShader) {
			...
			target.depthPlane = EditorGUILayout.ObjectField(GUIContent("Depth Plane","Assign Depth Plane Prefab"),target.depthPlane,GameObject,allowSceneObjects);
